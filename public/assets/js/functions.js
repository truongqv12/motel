    ...
    SEMICOLON.widget = {
                ...
                $flexSliderEl.each(function () {
                    ...
                    $flexsSlider.flexslider({
                        ...
                        start: function (slider) {
                            ...
                            SEMICOLON.initialize.lightbox();
                            $('.flex-prev').html('<i class="icon-caret-left"></i>');
                            $('.flex-next').html('<i class="icon-caret-right"></i>');
        ...
        cartQuantity: function () {
            ...
            $(".plus").off('click').on('click', function () {
                ...
                var inputQty = $(this).parents('.quantity').find('.qty[data-id=' + pro_id + ']');
                var minus = $(this).parents('.quantity').find('.minus[data-id=' + pro_id + ']');

                var productQuantity = inputQty.val(),
                    intRegex = /^\d+$/;
                var max = inputQty.attr('max');

                if (Number(productQuantity) === Number(max)) {
                    $(".plus[data-id='" + pro_id + "']").attr('disabled', true);
                }
                else if (intRegex.test(productQuantity) && Number(productQuantity) >= 1) {
            ...
            $(".minus").off('click').on('click', function () {
                var pro_id = $(this).data('id');
                var inputQty = $(this).parents('.quantity').find('.qty[data-id=' + pro_id + ']');
                var plus = $(this).parents('.quantity').find('.plus[data-id=' + pro_id + ']');
                var productQuantity = inputQty.val(),
                    intRegex = /^\d+$/;
                var max = Number(inputQty.attr('max'));

                if (intRegex.test(productQuantity) && Number(productQuantity) <= max) {
                    if (Number(productQuantity) > 1) {
                        plus.removeAttr('disabled')
                        var productQuantityMinus = Number(productQuantity) - 1;
                        inputQty.val(productQuantityMinus);
                    }
                    else if (Number(productQuantity) === 1) {
                        $(".minus[data-id='" + pro_id + "']").attr('disabled', true);
                    }
                } else {
                    inputQty.val(1);
                }
